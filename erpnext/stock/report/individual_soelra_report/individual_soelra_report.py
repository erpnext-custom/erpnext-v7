...
def get_data(filters):
	...
	query = frappe.db.sql(""" 
			...
			Where se.docstatus = 1 and se.purpose = 'Material Issue' and se.stock_entry_type = 'Soelra' {0}
...
def get_condition(filters):
	condition = ''
	if filters.desuupid:
		condition = " and issued_to = '{0}'".format(filters.desuupid)
	
	if filters.others:
		condition = " and issued_to_other like '%{0}%'".format(filters.others)
