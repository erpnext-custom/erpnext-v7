...
from frappe import _
from frappe.utils import flt, getdate, formatdate, cstr
# location column added by Birendra on 26/03/2021
def execute(filters=None):
	validate_filters(filters)
	data = get_data(filters)
	columns = get_columns()
...
def get_data(filters):
	query = """
	select distinct dn.transporter_name1,
		dn.lr_no, dn.lr_date, dn.customer, 
		dn.name, 
		dn.posting_date, 
		dni.item_code, 
		dni.item_name, 
		sii.delivered_qty, 
		sii.accepted_qty, 
		(ifnull(sii.delivered_qty, 0) - ifnull(sii.accepted_qty,0)) as difference_qty, 
		sii.abnormal_loss, 
		sii.normal_loss, 
		sii.abnormal_loss_amt, 
		sii.normal_loss_amt, 
		case 
			when sii.loss_method = 'Quantity in Flat' 
			then sii.loss_qty_flat else concat(sii.loss_tolerance, '%') 
			end as loss_tolerance, 
		sii.parent as sales_invoice, 
		(select si.posting_date from `tabSales Invoice` as si where si.name = sii.parent) as si_date, 
		(select si.sales_invoice_date from `tabSales Invoice` as si where si.name = sii.parent) as ref_date, 
		sii.loss_method, 
		sii.remarks, 
		sii.justification,
		dn.location
