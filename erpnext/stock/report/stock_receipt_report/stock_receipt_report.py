...
def get_columns(filters):
	cols = [
		...
		("To Warehouse")+":data:150",
		("UoM") + ":data:50",
		("Qty")+":Float:50",
		("Valuation Rate")+":data:120",
		("Amount")+":Currency:110",
		("Lot Number")+ ":data:100",
		("From Warehouse")+":data:170"
...
def get_data(filters):
	data = "select se.posting_date, sed.item_code, sed.item_name, (select i.item_group from tabItem i where i.item_code = sed.item_code) as item_group, (select i.item_sub_group from tabItem i where i.item_code = sed.item_code) as item_sub_group, sed.t_warehouse, sed.uom, sed.qty, sed.valuation_rate,sed.amount, sed.lot_list, sed.s_warehouse, se.name FROM `tabStock Entry` se, `tabStock Entry Detail` sed WHERE se.name = sed.parent and  se.docstatus = 1"
	if filters.cost_center:
		if not filters.get("branch"):
			all_ccs = get_child_cost_centers(filters.cost_center)
			data += " and se.to_warehouse in (select parent from `tabWarehouse Branch` wb where wb.branch in (select name from `tabBranch` b where b.cost_center in {0}))".format(tuple(all_ccs))
		else:
			branch_name = frappe.db.get_value("Branch",{"cost_center": filters.get("branch")}, "name")
			data += " and se.to_warehouse in (select parent from `tabWarehouse Branch` wb where wb.branch in (select name from `tabBranch` b where b.name = '{0}'))".format(branch_name)
