...
def get_data(filters):
	query = """
			SELECT a.name, a.asset_name, a.asset_category, a.asset_sub_category,
			...
			a.residual_value,
			(
				(CASE WHEN a.status in ('Scrapped', 'Sold') AND a.disposal_date < '{from_date}' THEN 0
					WHEN a.purchase_date < '{from_date}' THEN IFNULL(a.asset_rate,0)*IFNULL(a.asset_quantity_,1)
					ELSE 0 END)
				+
				(CASE WHEN a.status in ('Scrapped', 'Sold') AND a.disposal_date < '{from_date}' THEN 0
					ELSE (
