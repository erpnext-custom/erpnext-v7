...
class AssetMovement(Document):
    def validate(self):
        ...
        if self.target_custodian:
            ...
            self.target_cost_center = None
            self.target_custodian_cost_center = frappe.db.get_value(
                "Employee", self.target_custodian, "cost_center")
    ...
    def assign_data(self):
        self.source_custodian = frappe.db.get_value(
            "Asset", self.asset, "issued_to")
        self.current_cost_center = frappe.db.get_value(
            "Asset", self.asset, "cost_center")
        self.current_business_activity = frappe.db.get_value(
            "Asset", self.asset, "business_activity")

    def validate_asset(self):
        status, company = frappe.db.get_value(
            "Asset", self.asset, ["status", "company"])
        if status in ("Draft", "Scrapped", "Sold"):
            frappe.throw(_("{0} asset cannot be transferred").format(status))

        if company != self.company:
            frappe.throw(_("asset {0} does not belong to company {1}").format(
                self.asset, self.company))

    def validate_warehouses(self):
        if not self.source_warehouse:
            self.source_warehouse = frappe.db.get_value(
                "Asset", self.asset, "warehouse")
    ...
    def on_submit(self):
        if self.target_business_activity:
            set_business_activity(
                self.asset, self.current_business_activity, self.target_business_activity, True)
            # return
        ...
        if self.target_custodian:
            ...
            if self.target_custodian_cost_center != self.current_cost_center:
                make_asset_transfer_gl(self, self.asset, self.posting_date,
                                       self.current_cost_center, self.target_custodian_cost_center)
        if self.target_cost_center:
            self.set_latest_cc_in_asset()
            make_asset_transfer_gl(self, self.asset, self.posting_date,
                                   self.current_cost_center, self.target_cost_center)

    def on_cancel(self):
        if self.target_business_activity:
            set_business_activity(
                self.asset, self.current_business_activity, self.target_business_activity, False)
            # return
    ...
    def check_gls(self):
        jes = frappe.db.sql("select je.name from `tabJournal Entry` je, `tabJournal Entry Account` jea where je.name = jea.parent and jea.reference_name = %s and je.posting_date between %s and %s",
                            (self.asset, self.posting_date, getdate(nowdate())), as_dict=True)
        for a in jes:
            frappe.throw(
                "Cannot cancel since asset depreciations had already taken place")

    def cancel_gl_entries(self):
        frappe.db.sql(
            "delete from `tabGL Entry` where voucher_no = %s", self.name)
    ...
    def set_latest_custodian_in_asset(self, cancel=None):
        ...
        if self.current_cost_center != self.target_custodian_cost_center:
            ...
            frappe.db.set_value("Asset", self.asset, "cost_center", frappe.db.get_value(
                "Employee", custodian, "cost_center"))
            frappe.db.set_value("Asset", self.asset, "branch", branch)

            equipment = frappe.db.get_value(
                "Equipment", {"asset_code": self.asset}, "name")
            if equipment:
                save_equipment(equipment, branch,
                               self.posting_date, self.name, purpose)
...
def set_business_activity(asset, source_ba, target_ba, submit):
    ...
    equipment = frappe.db.get_value(
        "Equipment", {"asset_code": asset.name}, "name")
    if equipment:
