...
class ProjectInvoice(AccountsController):
	...
	def validate_items(self):
		...
		self.net_invoice_amount      = flt(self.gross_invoice_amount)+flt(self.price_adjustment_amount)-flt(self.advance_recovery)-flt(self.total_deduction_amount)
	...
	def make_gl_entries(self):
		if self.net_invoice_amount:
			...
			if self.party_type == "Customer":
				...
				if self.net_invoice_amount < 0: #added by Jai 6 Oct 2021
				   rec_gl = frappe.db.get_value(doctype="Projects Accounts Settings",fieldname="refundable_account_supplier")
			else:
			...
			gl_entries.append(
				self.get_gl_dict({
					   "account":  inv_gl,
					   "against": self.party,
					   "debit" if self.party_type == "Supplier" else "credit": self.gross_invoice_amount,
					   "debit_in_account_currency" if self.party_type == "Supplier" else "credit_in_account_currency": self.gross_invoice_amount,
					   "project": self.project,
					   "cost_center": self.cost_center,
					   "business_activity": default_ba
				}, self.currency)
...
def get_payment_entry(doc_name, total_amount, project, party, party_type):
		...
		payment_entry = """
				...
				WHERE   
						docstatus = 1 and
						reference_name = '{reference}' and 
						project = '{name}' and 
						party_type = '{type}' and 
						party = '{party}'
				""".format(reference=doc_name, name=project, type=party_type, party=party)
