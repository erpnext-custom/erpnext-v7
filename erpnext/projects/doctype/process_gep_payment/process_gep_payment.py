...
def get_records(from_date, to_date, branch, project=None):
	...
	ans = frappe.db.sql("select a.name, a.person_name, a.id_card, a.salary, a.ot_amount, (select sum(c.number_of_hours) from `tabOvertime Entry` c where c.number = a.name and c.date between %s and %s and c.docstatus = 1) as number_of_hours from `tabDES Employee` a where %s order by a.person_name", (str(from_date), str(to_date), condition), as_dict=True)
