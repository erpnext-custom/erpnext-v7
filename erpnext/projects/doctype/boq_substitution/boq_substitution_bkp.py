...
class BOQSubstitution(Document):
	...
	def substitute_boq_item(self):
			...
			if d.substitute:
				...
				boq_item.submit()
				frappe.db.sql(""" update `tabBOQ Item` set parentfield = "initial_boq_item" where name = '{0}'""".format(d.boq_item_name))

	def reset_boq_items(self):
		for d in self.boq_item:
			frappe.db.sql(""" delete from `tabBOQ Item` where ref_name = '{0}' """.format(d.boq_item_name))
			new_doclist = get_mapped_doc("Initial BOQ", d.boq_item_name, {
				"Initial BOQ": {
					"doctype": "BOQ Item",
					"field_map": {
						"ref_name": "name",
	...
	def update_boq_and_project(self):
			...
			frappe.db.commit()
                        ...
                        pro_doc.project_value = flt(pro_doc.project_value) + flt(self.implication_amount) * flt(mul_factor)
