frappe.listview_settings['Project'] = {
	...
	get_indicator: function(doc) {
		if(parseFloat(doc.tot_wq_percent_complete) < parseFloat(doc.tot_wq_percent)){
			...
			return [__("{0}% Complete", [parseFloat(doc.tot_wq_percent_complete)]), "orange", "tot_wq_percent_complete,>=,0|status,=,Ongoing"];
		} else if(parseFloat(doc.tot_wq_percent_complete) >= parseFloat(doc.tot_wq_percent)){
			return [__("{0}% Complete", [parseFloat(doc.tot_wq_percent_complete)]), "green", "tot_wq_percent_complete,>=,0|status,=,Ongoing"];
		} else {
			return [__(doc.status), frappe.utils.guess_colour(doc.status), "statusx,=," + doc.status];
		}
	},
};
