...
def check_budget_available(cost_center, budget_account, transaction_date, amount, business_activity):
	if frappe.db.get_value("Account", budget_account, "budget_check"):
        ...
        budget_amount = frappe.db.sql("""select b.action_if_annual_budget_exceeded as action, ba.budget_check, ba.budget_amount 
                                ...
                                """, (budget_account, cost_center, fiscal_year[0]["name"], business_activity), as_dict=True)
	...
	ig_or_stop = budget_amount and budget_amount[0].action or None
        ...
        if ig_or_stop == "Ignore" or ig_or_stop_gl == "Ignore":
                ...
                if budget_amount:
                        committed = frappe.db.sql("select SUM(cb.amount) as total from `tabCommitted Budget` cb where cb.cost_center=%s and cb.account=%s and cb.po_date between %s and %s and cb.business_activity =%s", (cost_center, budget_account, fiscal_year[0]["year_start_date"], fiscal_year[0]["year_end_date"], business_activity), as_dict=True)
                        consumed = frappe.db.sql("select SUM(cb.amount) as total from `tabConsumed Budget` cb where cb.cost_center=%s and cb.account=%s and cb.po_date between %s and %s and cb.business_activity =%s", (cost_center, budget_account, fiscal_year[0]["year_start_date"], fiscal_year[0]["year_end_date"], business_activity), as_dict=True)
