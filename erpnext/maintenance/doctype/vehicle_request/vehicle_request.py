...
class VehicleRequest(Document):
	def validate(self):
		...
		if self.workflow_state in ("Rejected", "Rejected by Supervisor", "Rejected By MTO") and self.rejection_reason == None:
                        frappe.throw("Specify the Rejection/Cancellation Reson")

	def on_submit(self):
		if not self.equipment:
                        frappe.throw("Equipment is not assigned")
		...
		if self.approver:
			...
			frappe.throw("Set Reports To Field in Employee Master")
                ...
                if self.approver_id != frappe.session.user:
                        frappe.throw("Only the selected supervisor can {0} this document".format(self.workflow_state))
