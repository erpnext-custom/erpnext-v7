...
def get_data(filters):
	...
	if filters.get("category"):
		...
		cat = " and e.equipment_category = e.equipment_category"
	...
	equipments = frappe.db.sql("""
		select 
			e.name as name, eh.branch as branch, e.hsd_type, e.equipment_number as equipment_number, 
	...
	for eq in equipments:
		query = """ 
				select 
					'{0}' as name, '{1}' as ty, '{2}' as no, '{3}' as br, vl.branch, 
					ifnull(MIN(vl.initial_km),0)  AS mink, ifnull(MAX(vl.final_km),0) AS maxk, 
					ifnull(MIN(vl.initial_hour),0) as minh, ifnull(MAX(vl.final_hour),0) as maxh, 
					ifnull(vl.consumption_km,0) as ckm, ifnull(vl.consumption_hours,0) as ch,
					( select ifnull((sum(pol.qty*pol.rate)/sum(pol.qty)),0) 
						from tabPOL pol 
						where pol.equipment_number = '{2}' and pol.posting_date between '{7}' and '{8}'
					) as rate,
					(select em.tank_capacity 
						from  `tabEquipment Model` em 
						where em.name = '{5}'
					) as cap, 
					CASE
					...
					END as yshour,
					ifnull(sum(vl.distance_km),0) as km, ifnull(sum(vl.consumption),0) as consumed 
				from
					`tabVehicle Logbook` vl 
				where vl.equipment_number = '{2}' {6} and vl.docstatus = 1 having sum(vl.consumption)>0
			""".format(eq.name, eq.equipment_type, eq.equipment_number, eq.branch, eq.hsd_type, eq.equipment_model, vl_date, from_date, to_date)

		datas = frappe.db.sql(query, as_dict=1)
		for d in datas:
			d.drawn = get_pol_between("Receive", d.name, filters.from_date, filters.to_date, d.hsd_type)
			received_till = get_pol_till("Receive", d.name, filters.from_date, filters.to_date, d.hsd_type)
			consumed_till = get_pol_consumed_till(d.name, filters.from_date, filters.to_date)
