...
def get_columns(filters=None):
	if filters.aggregate:
		if filters.report_by == "Sales Order":
			columns = [
				_("Branch") + ":Link/Sales Order:150", _("Location") + ":Data/120", _("Sub Item Group") + ":Data:150", _("Sales Qty") + ":Float:120",_("Delivered Qty") + ":Float:120", _("Amount") + ":Currency:120"
			]

		else:
			columns = [
                                _("Branch") + ":Link/Sales Order:150", _("Location") + ":Data/120", _("Sub Item Group") + ":Data:150", _("Delivered Qty") + ":Float:120", _("Amount") + ":Currency:120"
                        ]
	else:
		if filters.report_by == "Sales Order":
			columns = [
				  _("Sales Order") + ":Link/Sales Order:100", _("Branch") + ":Link/Branch:120", _("Posting Date") + ":Date:100", 
				  _("Item Code") + ":Link/Item: 80", _("Timber Class") + ":Link/Timber Class:100", _("Timber Species") + ":Link/Timber Species:100", _("Timber Type") + ":Data:100", _("Lot No.") + ":Data:100", _("Item Name") + ":Data:150", _("Sub Group") + ":Data:100",
			...
			columns = [
				  ...
				  _("Posting Date") + ":Date:100", _("Item Code") + ":Link/Item: 80", _("Timber Class") + ":Link/Timber Class:100", _("Timber Species") + ":Link/Timber Species:100", _("Timber Type") + ":Data:100", _("Lot No.") + ":Data:100", _("Item Name") + ":Data:150", _("Sub Group") + ":Data:100",
...
def get_data(filters=None):
	...
	if filters.report_by == "Sales Order":
		if filters.aggregate:
			...
			cols = "so.name, so.branch, so.transaction_date, soi.item_code, ts.timber_class,ts.species,ts.timber_type, soi.lot_number, soi.item_name, i.item_sub_group, so.customer, soi.qty as qty, soi.delivered_qty, soi.rate, soi.amount"
			group_by = "and 1 = 1"
