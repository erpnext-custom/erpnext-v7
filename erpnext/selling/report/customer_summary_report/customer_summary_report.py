...
def get_data(filters):
	data = []
	row = []
	query = """ select si.name, si.customer, sii.item_code, sii.item_name, sii.item_type, sum(case when si.posting_date < '{from_date}' then ifnull(si.outstanding_amount, 0) else 0 end) as opening, sum(case when si.posting_date between '{from_date}' and '{to_date}' then ifnull(sii.accepted_qty, 0) else 0 end) as accepted_qty, sum(case when si.posting_date between '{from_date}' and '{to_date}' then ifnull(sii.amount + sii.excess_amt + si.charges_total + sii.normal_loss_amt + sii.abnormal_loss_amt, 0) else 0 end) as billed_amount from `tabSales Invoice` si, `tabSales Invoice Item` sii where sii.parent = si.name and si.docstatus = 1""".format(from_date = filters.from_date, to_date = filters.to_date)
...
def get_customer_details(filters, customer):
	cust = frappe.db.sql(""" select territory, market from `tabCustomer` where name = "{0}" """.format(customer), as_dict = 1)
	if cust:
		return cust[0].territory, cust[0].market
	else:
		 return ''
