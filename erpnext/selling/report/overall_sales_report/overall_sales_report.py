	...
	if filters.report_by == "Sales Order":
		...
		if filters.aggregate:
			...
			cols = """
				...
				dn.discount_or_cost_amount, dn.additional_cost, sum(dni.net_amount)-(dn.challan_cost)/count(select a.name from `tabDelivery Note Item` a where a.parent = dn.name group by a.item_group)-(dn.loading_cost)/count(select a.name from `tabDelivery Note Item` a where a.parent = dn.name group by a.item_group),
