	...
	if filters.report_by == "Sales Order":
		...
		if filters.aggregate:
			...
			cols = """
				...
				dn.discount_or_cost_amount, dn.additional_cost, sum(dni.net_amount)-(dn.challan_cost)/(select count(data.name) from (select a.name as name `tabDelivery Note Item` a where a.parent = dn.name group by a.item_code) data)-(dn.loading_cost)/(select count(data.name) from (select a.name as name `tabDelivery Note Item` a where a.parent = dn.name group by a.item_code) data),
