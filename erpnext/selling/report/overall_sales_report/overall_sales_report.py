	...
	if filters.report_by == "Sales Order":
		...
		if filters.aggregate:
			cols = """
				...
				sum(dni.amount) - (dni.discount_amount) + sum(dni.additional_cost) - sum(dn.challan_cost)
			"""
			...
			cols = """
				...
				dn.discount_or_cost_amount, dn.additional_cost, sum(dni.amount) - sum(dni.discount_amount) + sum(dni.additional_cost) - dn.challan_cost,
