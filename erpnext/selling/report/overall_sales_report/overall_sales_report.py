	...
	if filters.report_by == "Sales Order":
		...
		if filters.aggregate:
			...
			cols = """
				...
				dn.discount_or_cost_amount, dn.additional_cost, sum(dni.net_amount)-(dn.challan_cost)/(select count(data.name) from (select a.name from `tabDelivery Note Item` a where a.name = dn.parent group by a.item_group) data)-(dn.loading_cost)/count(dn.name),
