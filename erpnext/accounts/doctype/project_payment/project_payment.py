...
class ProjectPayment(AccountsController):
	...
	def update_invoice_balance(self):
		for inv in self.references:
			...
			if flt(inv.allocated_amount) > 0:
				...
				# if flt(total_balance_amount) < flt(inv.allocated_amount) and self.docstatus != 2:#commented by Jai, 6 Oct 2021
	...
	def validate_invoice_balance(self):
		for inv in self.references:
			if flt(inv.allocated_amount) > 0:
				total_balance_amount = frappe.db.get_value("Project Invoice", inv.reference_name, "total_balance_amount")
				# if flt(total_balance_amount) < flt(inv.allocated_amount): #commented by Jai, 6 Oct 2021
				if flt(inv.total_amount) < flt(inv.allocated_amount):
	...
	# NOT NEEDED SINCE IT IS NOW IN PROJECT INVOICE
	...
	#         else:
	#                 self.tds_amount = flt((self.tds_rate / flt(100))) * self.tds_taxable_amount
	...
	def validate_allocated_amounts(self):
		...
		if round(flt(tot_adv_amount),2) > round(flt(tot_inv_amount),2):
		...
		if round(flt(self.total_amount),2) > round(flt(tot_inv_amount),2):
	...
	def make_gl_entries(self):
		if flt(self.total_amount) > 0:
			...
			if flt(self.paid_amount) > 0:
				...
				project_invoice = frappe.db.get_value("Project Invoice", self.reference_name, "net_invoice_amount")
				if project_invoice < 0: #added by Jai, 6 Oct 2021
						gl_entries.append(
			...
			if flt(self.total_amount) > 0:
				...
				project_invoice = frappe.db.get_value("Project Invoice", self.reference_name, "net_invoice_amount")
				if project_invoice < 0: #added by Jai, 6 Oct 2021
						sundry_account = frappe.db.get_value(doctype="Projects Accounts Settings",fieldname="refundable_account_supplier")
...
def get_invoice_list(project, party_type, party):
    # below code commented by Jai, taken out total_balance_amount > 0, 5 Oct 2021
