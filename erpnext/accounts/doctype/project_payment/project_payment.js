...
frappe.ui.form.on('Project Payment', {
	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.references && frm.doc.paid_amount == 0) {
			let total_amount = 0
			frm.doc.references.map(item => {
				total_amount += item.total_amount
			})
			frm.doc.paid_amount = total_amount
...
// commented out by phuntsho on april 27 2021. Not needed with new TDS calculation 
...
cur_frm.cscript.type = function (doc) {
	...
	if (!doc.tds_taxable_amount) {
		...
		cur_frm.doc.references.map(item => {
			total_invoice_amount += item.total_amount
		})
...
// Following code added by SHIV on 2019/06/19, row.total_amount modified by Jai
			...
			callback: function (r) {
				if (r.message) {
					...
					r.message.forEach(function (inv) {
						...
						row.total_amount = (flt(inv['total_balance_amount']) > 0)? flt(inv['total_balance_amount']) : -(flt(inv['total_balance_amount']));
