...
def get_transporter_rate(warehouse, date, equipment_type, item_code):
	for d in frappe.db.sql("select a.name, b.threshold_trip, b.lower_rate, b.higher_rate, b.unloading_rate, a.expense_account from `tabTransporter Rate` a, `tabTransporter Rate Item` b  where a.name = b.parent and  %s between a.from_date and a.to_date and a.receiving_warehouse = %s and a.equipment_type = %s and b.item_code = %s", (date, warehouse, equipment_type, item_code), as_dict=1):
		return {"name": d.name, "lower_rate": d.lower_rate, "higher_rate": d.higher_rate, "unloading_rate": d.unloading_rate, "threshold_trip": d.threshold_trip, "expense_account": d.expense_account}
