...
class TransporterPayment(AccountsController):
	...
	def calculate_on_save(self):
		...
		unloading_amount = 0
		transfer_charges = 0.0
		delivery_charges  = 0.0
		for i in self.items:
			if i.reference_type == 'Stock Entry':
				transfer_charges += i.transportation_amount
				transfer_acc = i.expense_account

			if i.reference_type == 'Delivery Note':
				delivery_charges += i.transportation_amount
				del_acc = i.expense_account
		...
		self.transfer_charges = transfer_charges
		self.delivery_charges  = delivery_charges
		self.transportation_amount = total_transporter_amount
		self.delivery_account = del_acc
		self.transfer_account = transfer_acc
	...
	def calculate_total(self):
		...
		if self.security_deposit_percent:
			self.security_deposit_amount = flt(self.gross_amount) * flt(self.security_deposit_percent) / 100.0
                        self.security_deposit_account = frappe.db.get_value("Accounts Settings", None, "secutiry_deposit_account") 
	...
	def get_payment_details(self):
		...
		'''
		#cond = " 1 = 1"
		#if self.receiving_warehouse:
		#	cond = " b.receiving_warehouse = '{0}'".format(self.receiving_warehouse)
		
		stock_transfer = frappe.db.sql("""select b.name as reference_row, a.posting_date, 'Stock Entry' as reference_type, a.name as reference_name, b.item_code, b.item_name, b.t_warehouse as receiving_warehouse, b.received_qty as qty, a.unloading_by, a.equipment from `tabStock Entry` a, `tabStock Entry Detail` b where a.name = b.parent and a.docstatus = 1 and a.transport_payment_done = 0 and a.purpose = 'Material Transfer' and a.posting_date between '{0}' and '{1}' and a.equipment = '{2}'""".format(self.from_date, self.to_date, self.equipment), as_dict = True)
	
		delivery_note  = frappe.db.sql("""select b.name as reference_row, a.posting_date, 'Delivery Note' as reference_type, a.name as reference_name, b.item_code, b.item_name, a.customer as receiving_warehouse, b.qty as qty, '' as unloading_by, a.equipment from `tabDelivery Note` a, `tabDelivery Note Item` b where a.name = b.parent and a.docstatus = 1 and a.posting_date between '{0}' and '{1}' and a.equipment = '{2}' and transport_payment_done = 0""".format(self.from_date, self.to_date, self.equipment), as_dict = True)
		...
		for d in entries:
			...
			d.expense_account = doc['expense_account']
	...
	def check_budget(self):
		...
		if trans_amount:
			check_budget_available(cc, self.delivery_account, self.posting_date, trans_amount)
			check_budget_available(cc, self.transfer_account, self.posting_date, trans_amount)
	...
	def make_gl_entry(self):
		...
		frappe.msgprint("this is")
		if flt(self.transfer_charges +  self.delivery_charges) > 0:
			frappe.msgprint("{0}".format(self.transfer_charges)) 
                        if self.transfer_charges:
				gl_entries.append(
                                	self.get_gl_dict({
                                       		"account":  self.transfer_account,
                                       		"debit": self.transfer_charges,
                                       		"debit_in_account_currency": self.transfer_charges,
                                       		"against_voucher": self.name,
                                       		"against_voucher_type": self.doctype,
                                       		"cost_center": cost_center,
                                	}, self.currency)
			...
			if self.delivery_charges:
                                gl_entries.append(
                                        self.get_gl_dict({
                                                "account":  self.delivery_account,
                                                "debit": self.delivery_charges,
                                                "debit_in_account_currency": self.delivery_charges,
                                                "against_voucher": self.name,
                                                "against_voucher_type": self.doctype,
                                                "cost_center": cost_center,
                                        }, self.currency)
