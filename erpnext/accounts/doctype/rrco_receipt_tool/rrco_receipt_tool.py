            ...
            if self.item:
                for a in self.item:
                    ...
                    if a.transaction == "Direct Payment":
                        ...
                        bill_no = a.invoice_no
                    elif a.transaction == "Hire Charge Invoice":
                        bill_no = a.invoice_no
                    elif a.transaction == "Mechanical Payment":
                        bill_no = a.invoice_no
    ...
    def get_invoices(self):
        if self.purpose in ["Leave Encashment", "Purchase Invoices"]:
            ...
            if self.purpose == 'Leave Encashment':
                ...
                query = """
                    SELECT
                        "Purchase Invoice" as transaction, p.name, p.posting_date, 
                        p.tds_taxable_amount as invoice_amount, p.tds_amount as tax_amount, 
                        p.supplier as party
                    FROM `tabPurchase Invoice` AS p 
                    WHERE 
                        p.docstatus = 1 AND p.posting_date BETWEEN '{0}' AND '{1}' 
                        AND p.tds_rate = '{2}' 
                        AND NOT EXISTS (SELECT 1 
                                        FROM `tabRRCO Receipt Entries` AS b 
                    ...
                    UNION 
                    SELECT
                        "Direct Payment" as transaction, d.name, d.posting_date,
                        di.taxable_amount as invoice_amount, di.tds_amount as tax_amount, 
                        di.party as party 
                    FROM `tabDirect Payment` AS d, `tabDirect Payment Item` di 
                    WHERE 
                        di.parent = d.name AND d.docstatus = 1 
                    ...
                    UNION 
                    SELECT
                        "Hire Charge Invoice" as transaction, h.name, h.posting_date, 
                        h.total_invoice_amount as invoice_amount, h.tds_amount as tax_amount,
                        h.customer as party
                    FROM `tabHire Charge Invoice` AS h 
                    WHERE h.docstatus = 1 
                        AND h.posting_date BETWEEN '{0}' AND '{1}'
                        AND h.tds_percentage = '{2}' 
                        AND NOT EXISTS (SELECT 1 
                                        FROM `tabRRCO Receipt Entries` AS b 
                                        WHERE b.purchase_invoice = h.name)
                        AND EXISTS (
							select 1
							from `tabTDS Remittance`  as r, `tabTDS Remittance Item` ri
							where r.name = ri.parent
							and r.docstatus = 1 
       						and ri.invoice_no = h.name
						)
                    UNION 
                    SELECT 
                        "Mechanical Payment" as transaction, m.name, m.posting_date, 
                        m.payable_amount as invoice_amount, m.tds_amount as tax_amount,
                        m.supplier as party
                    FROM `tabMechanical Payment` AS m
                    WHERE m.docstatus = 1 
                    AND m.posting_date BETWEEN '{0}' AND '{1}'
                    AND m.tds_rate = '{2}%' 
                    AND NOT EXISTS (SELECT 1 
                                    FROM `tabRRCO Receipt Entries` AS b 
                                    WHERE b.purchase_invoice = m.name)
                        AND EXISTS (
							select 1
							from `tabTDS Remittance`  as r, `tabTDS Remittance Item` ri
							where r.name = ri.parent
							and r.docstatus = 1 
       						and ri.invoice_no = m.name
						)				
...
#rrco.branch = self.branch
#rrco.branch = self.branch
#rrco.branch = str(self.branch)
#not self.branch and
#AND a.branch ='{2}' self.branch  AND a.branch = '{3}'  	AND a.branch = '{3}' 