...
def validate_expense_against_budget(args):
	...
	if args.against_voucher_type == 'Asset':
		...
		if budget_amount:
			consumed = frappe.db.sql("select SUM(cb.amount) as total from `tabCommitted Budget` cb where cb.cost_center=%s and cb.account=%s and cb.po_date between %s and %s", (args.cost_center, args.account, str(self.posting_date)[0:4] + "-01-01", str(self.posting_date)[0:4] + "-12-31"), as_dict=True)
			if consumed:
