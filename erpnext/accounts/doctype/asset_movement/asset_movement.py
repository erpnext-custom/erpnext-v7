...
from erpnext.accounts.asset_utils import check_valid_asset_transfer
	...
	def on_submit(self):
		...
		if self.target_custodian:
			...
			if self.target_custodian_cost_center != self.current_cost_center:		
				pass #make_asset_transfer_gl(self, self.asset, self.posting_date, self.current_cost_center, self.target_custodian_cost_center)
		if self.target_cost_center:	
			self.set_latest_cc_in_asset()
			pass #make_asset_transfer_gl(self, self.asset, self.posting_date, self.current_cost_center, self.target_cost_center)
