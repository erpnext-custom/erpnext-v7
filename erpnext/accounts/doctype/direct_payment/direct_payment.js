...
frappe.ui.form.on('Direct Payment', {
	...
	"tds_percent": function(frm) {
		...
		frm.set_value("net_amount", parseFloat(net_amt) - parseFloat(frm.doc.deduction_amount));
...
function calculate_total(frm, cdt, cdn){
	...
	var gross_amount=0, total_taxable_amount=0, total_net_amount=0, total_tds_amount=0, total_deduction=0;
	frm.doc.item.forEach(function(d) {
	...
	frm.doc.deduct.forEach(function(d) {
		total_deduction += parseFloat(d.amount);
		cur_frm.set_value("deduction_amount", total_deduction);
	});
	
	frm.set_value("amount", gross_amount);
	frm.set_value("taxable_amount", total_taxable_amount);
	frm.set_value("net_amount", parseFloat(total_net_amount) - parseFloat(total_deduction));
