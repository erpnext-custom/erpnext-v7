...
class OpeningInvoiceCreationTool(Document):
	...
	def get_invoice_dict(self, row=None):
		def get_item_dict():
			...
			# Ver 3.0 Begins, Following code added by SHIV on 2018/10/24
			if not row.cost_center:
                                frappe.throw(_("Row {0}: Cost Center is mandatory.").format(row.idx),title="Missing Data")                                
			# Ver 3.0 Ends
		...
		item = get_item_dict()
                ...
                branch = frappe.db.get_value("Branch", {"cost_center": row.cost_center}, "name") # Added by SHIV on 2018/10/24
