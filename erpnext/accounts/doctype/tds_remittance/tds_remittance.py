...
class TDSRemittance(AccountsController):
	...
	def get_details(self):
		query = """ select d.posting_date, d.party, d.name as invoice_no, d.taxable_amount as bill_amount, d.tds_amount from 
				...
				union all
				select t.posting_date, t.equipment as supplier, t.name,  t.gross_amount as bill_amount, t.tds_amount 
                                from `tabTransporter Payment` t where t.tds_percent = '{0}' and t.docstatus =1 
                                and t.posting_date >= '{1}' and t.posting_date<= '{2}'
                                and not exists (
                                        select 1 from `tabTDS Remittance Item` i
                                        inner join `tabTDS Remittance` tr
                                        on i.parent = tr.name
                                        where i.invoice_no = t.name
                                        and tr.docstatus = 1)
