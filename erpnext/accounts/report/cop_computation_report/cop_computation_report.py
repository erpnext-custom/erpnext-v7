...
def set_gl_entries_by_account(filters, cost_center, company, from_date, to_date, root_lft, root_rgt, gl_entries_by_account,
                ignore_closing_entries=True, open_date=None):
        ...
        gl_entries = frappe.db.sql("""select posting_date, account, sum(debit) as debit, sum(credit) as credit, is_opening 
			from `tabGL Entry` where company=%(company)s {additional_conditions}
                	and account in 
				(select name from `tabAccount` where lft >= %(lft)s and rgt <= %(rgt)s) and cost_center = %(cost_center)s and 
				account in 
					( select c.account from `tabCOP Item` c, `tabCOP` p where c.parent = p.name and 
					p.cost_center = %(cost_center)s and posting_date between p.from_date and p.to_date)
