...
def get_result(filters, account_details, from_date, to_date):
	...
	#-------------------------------------------------------------------------------------------------------------------
	#Uncomment below to continue working on showing opening when bank or cash transactions are not there for the month Kinley Dorji
	# bank_entries = get_bank_entries(filters, from_date)
	# cash_entries = get_cash_entries(filters, from_date)
	# frappe.throw(str(cash_entries))
	# # if cash_entries == []:
...
def get_bank_entries(filters, from_date):
	select_fields = """, sum(debit_in_account_currency) as debit_in_account_currency,
		sum(credit_in_account_currency) as credit_in_account_currency""" \
		if filters.get("show_in_account_currency") else ""
...
def get_cash_entries(filters, from_date):
	select_fields = """, sum(debit_in_account_currency) as debit_in_account_currency,
		sum(credit_in_account_currency) as credit_in_account_currency""" \
		if filters.get("show_in_account_currency") else ""
	...
	cash_entries = frappe.db.sql("""
		select
			...
			a.is_opening {select_fields}
		from `tabGL Entry` a
		where a.docstatus = 1 and a.account in ('80.01-Cash in Hand - DS') and a.company=%(company)s {conditions}
		{group_by_condition}
		order by a.posting_date, a.account"""\
		.format(select_fields=select_fields, conditions=get_conditions(filters, from_date),
			group_by_condition=group_by_condition), filters, as_dict=1)
