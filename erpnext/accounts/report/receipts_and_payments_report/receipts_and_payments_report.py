...
from erpnext.accounts.report.financial_statement_ds \
...
def get_data(filters):
	accounts = frappe.db.sql("""select name,  parent_account, account_name, root_type, report_type, lft, rgt
		from `tabAccount` where company=%s and name not in ("Temporary Accounts - DS", 'Stock Assets - DS', 'Accumulated Depreciation - DS', 'Other Expenses - DS', 'Stock Expenses - DS', 'Stock Liabilities - DS', 'Other Incomes - DS', 'Inventories - DS', 'BOB-A/C: 202921909 - DS') order by name ASC""", filters.company, as_dict=True)
