...
from erpnext.accounts.report.financial_statements_emines \
	import filter_accounts, set_gl_entries_by_account1, filter_out_zero_value_rows
...
def get_data(filters):
	accounts = frappe.db.sql("""select name, account_code, parent_account, account_name, root_type, report_type, lft, rgt
		from `tabAccount` where company=%s and root_type != 'Liability' order by lft""", filters.company, as_dict=True)
	...
	set_gl_entries_by_account1(filters.cost_center, filters.company, filters.from_date,
...
def get_rootwise_opening_balances(filters, report_type):
	...
	gle = frappe.db.sql("""
		...
		where
			...
			and account in (select name from `tabAccount` where report_type=%(report_type)s and account_type in ('Bank', 'Cash')) 
