...
def get_period_list(fiscal_year, periodicity):
    ...
    fy_start_end_date = frappe.db.get_value("Fiscal Year", fiscal_year, [
...
def get_label(periodicity, from_date, to_date):
    if periodicity == "Yearly":
        if formatdate(from_date, "YYYY") == formatdate(to_date, "YYYY"):
            ...
            label = formatdate(from_date, "YYYY") + "-" + \
                formatdate(to_date, "YYYY")
    else:
        label = formatdate(from_date, "MMM YY") + "-" + \
            formatdate(to_date, "MMM YY")
...
def get_data(company, root_type, balance_must_be, period_list,
    ...
    calculate_values(accounts_by_name, gl_entries_by_account,
                     period_list, accumulated_values)
    accumulate_values_into_parents(
        accounts, accounts_by_name, period_list, accumulated_values)
            ...
            for period in period_list:
                ...
                if entry.posting_date <= period.to_date:
                    if accumulated_values or entry.posting_date >= period.from_date:
                        d[period.key] = d.get(
                            period.key, 0.0) + flt(entry.debit) - flt(entry.credit)
            ...
            for period in period_list:
                accounts_by_name[d.parent_account][period.key] = \
                    accounts_by_name[d.parent_account].get(
                        period.key, 0.0) + d.get(period.key, 0.0)
...
def filter_out_zero_value_rows(data, parent_children_map, show_zero_values=False):
    ...
    for d in data:
        if show_zero_values or d.get("has_value"):
            ...
            children = [child.name for child in parent_children_map.get(
                d.get("account")) or []]
            if children:
                for row in data:
