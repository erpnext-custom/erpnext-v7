...
def set_gl_entries_by_account(cost_center, company, from_date, to_date, root_lft, root_rgt, gl_entries_by_account,
	...
	### Added by Tashi Dorji to reduce the no of iteration based 
	if not cost_center:
                gl_entries = frappe.db.sql("""select max(posting_date) as posting_date, account, sum(debit) as debit, sum(credit) as credit, is_opening from `tabGL Entry`
                        where company=%(company)s 
                        {additional_conditions}
                        and account in (select name from `tabAccount`
                                where lft >= %(lft)s and rgt <= %(rgt)s) group by account, concat(MONTH(posting_date), YEAR(posting_date)), is_opening
