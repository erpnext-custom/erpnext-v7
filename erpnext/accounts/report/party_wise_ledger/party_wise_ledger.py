...
def get_data(filters, show_party_name):
	party_name_field = "customer_name, customer_type" if filters.get("party_type")=="Customer" else "supplier_name, supplier_type" if filters.get("party_type")=="Supplier" else"equipment_number" if filters.get("party_type")== "Equipment" else "registration_number" if filters.get("party_type") == "Vehicle" else "employee_name"
...
def get_data(filters, show_party_name):
	...
	if not filters.get("inter_company"):
		parties = frappe.get_all(filters.get("party_type"), fields = ["name", party_name_field, party_type], order_by="name")
	elif filters.get("party_type") in  ("Employee", "Equipment"):
		parties = frappe.get_all(filters.get("party_type"), fields = ["name", party_name_field], order_by="name")
	else:
		parties = frappe.get_all(filters.get("party_type"), fields = ["name", party_name_field], filters = {"inter_company": 1}, order_by="name")
