...
class COP(Document):
	...
	def check_duplicate(self):
		cop_list = frappe.db.sql(""" select count(name) tot from `tabCOP` where cost_center = "{0}" and name != '{3}' 
		and ('{1}' between from_date and to_date or '{2}' between from_date and to_date or ('{1}' > from_date and '{2}' < to_date) or ('{1}' < from_date and '{2}' > to_date)) """.format(self.cost_center, self.from_date, self.to_date, self.name), as_dict = 1)
