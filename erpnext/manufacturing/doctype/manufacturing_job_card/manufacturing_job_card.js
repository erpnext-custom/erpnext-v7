...
frappe.ui.form.on('Manufacturing Job Card', {
		...
		if (frm.doc.docstatus == 0) {
			...
			if (!frm.doc.job_started) {
				frm.add_custom_button(__("Start Job"), () => {
					let row = frappe.model.add_child(frm.doc, 'Manufacturing Job Card Time Log', 'time_logs');
	...
	make_material_request: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnext.manufacturing.doctype.manufacturing_job_card.manufacturing_job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnext.manufacturing.doctype.manufacturing_job_card.manufacturing_job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true
		});
	},
