...
class ManufacturingJobCard(Document):
	def validate(self):
	...
	def get_overlap_for(self, args):
		existing = frappe.db.sql("""select jc.name as name from
			`tabManufacturing Job Card Time Log` jctl, `tabManufacturing Job Card` jc where jctl.parent = jc.name and
			(
	...
	def on_submit(self):
		self.validate_manufacturing_job_card()
	...
	def validate_manufacturing_job_card(self):
	...
	def update_work_order(self):
		...
		for d in frappe.get_all('Manufacturing Job Card',
			filters = {'docstatus': 1, 'operation_id': self.operation_id}):
			doc = frappe.get_doc('Manufacturing Job Card', d.name)
	...
	def set_transferred_qty(self, update_status=False):
		...
		if self.items:
			self.transferred_qty = frappe.db.get_value('Stock Entry', {
				'manufacturing_job_card': self.name,
		...
		if self.work_order:
			...
			if doc.transfer_material_against == 'Manufacturing Job Card' and not doc.skip_transfer:
				completed = True
				...
				if completed:
					manufacturing_job_cards = frappe.get_all('Manufacturing Job Card', filters = {'work_order': self.work_order,
						'docstatus': ('!=', 2)}, fields = 'sum(transferred_qty) as qty', group_by='operation_id')

					if manufacturing_job_cards:
						qty = min([d.qty for d in manufacturing_job_cards])
...
def make_material_request(source_name, target_doc=None):
	...
	doclist = get_mapped_doc("Manufacturing Job Card", source_name, {
		"Manufacturing Job Card": {
			"doctype": "Material Request",
			"field_map": {
				"name": "manufacturing_job_card",
			},
		},
		"Manufacturing Job Card Item": {
