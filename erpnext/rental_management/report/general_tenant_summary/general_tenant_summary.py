...
def get_official_tenant_data(tenant,from_date,to_date):
	...
	query = frappe.db.sql("""
		...
		from (
			...
			from `tabRental Bill` rb 
			inner join `tabRental Payment Item` rpi on rpi.rental_bill = rb.name 
			inner join `tabRental Payment` rp on rpi.parent = rp.name 
			where rb.posting_date between '{0}' and '{1}' and rp.posting_date between '{0}' and '{1}' 
			and rpi.docstatus=1 and rb.docstatus=1 and rb.tenant = '{2}' order by rb.name
...
def get_data(filters):
	...
	query = frappe.db.sql("""
		...
		from (
			...
			inner join `tabRental Payment Item` rpi on rpi.rental_bill = rb.name 
			inner join `tabRental Payment` rp on rpi.parent = rp.name 
			where rb.posting_date between '{from_date}' and '{to_date}' and rp.posting_date between '{from_date}' and '{to_date}' 
			and rpi.docstatus=1 and rb.docstatus=1 order by rb.name
		) as x group by tenant
		""".format(from_date=filters.get("from_date"), to_date=filters.get("to_date")))
