...
def get_data(query, filters=None):
    ...
    for d in datas:
        row = [d.month, "Salary", d.basic_pay, round(flt(d.gross_pay) - flt(d.basic_pay) - (flt(d.comm_all) / 2), 2), round(flt(d.gross_pay)-(flt(d.comm_all) / 2), 2), round(flt(
            d.gross_pay)-(flt(d.comm_all) / 2), 2), d.nppf, d.gis, flt(d.gross_pay) - flt(d.nppf) - flt(d.gis) - (flt(d.comm_all) / 2), d.tds, d.health, d.receipt_number, d.receipt_date]
        data.append(row)

    # Leave Encashment
    if filters.employee:
        encash_data = frappe.db.sql("select distinct  a.application_date as date, a.encashment_amount, a.tax_amount, r.receipt_number, r.receipt_date from `tabLeave Encashment` a, `tabRRCO Receipt Entries` r where a.name = r.purchase_invoice and a.employee = %s and a.docstatus = 1 and a.application_date between \'" +
                                    filters.fiscal_year + "-01-01\' and \'" + filters.fiscal_year + "-12-31\'", filters.employee, as_dict=True)
        if encash_data:
            for a in encash_data:
                row = [str(a.date)[5:7], "Leave Encashment", a.encashment_amount, 0, a.encashment_amount,
                       a.encashment_amount, 0, 0, a.encashment_amount, a.tax_amount, 0, a.receipt_number, a.receipt_date]
                data.append(row)
        # Bonus
        bonus = frappe.db.sql("""
        ...
        for b in bonus:
            ...
            for a in amt:
                row = [str(b.posting_date)[5:7], "Bonus", a.amount, 0, a.amount, a.amount,
                       0, 0, a.amount, a.tax_amount, 0, b.receipt_number, b.receipt_date]
                data.append(row)
        # PVBA
        pbva = frappe.db.sql("""
					select distinct b.name, b.fiscal_year, b.posting_date, 
        ...
        for b in pbva:
            amt = frappe.db.sql("""
					...
					where parent = %s and employee = %s
				      """, (b.name, filters.employee), as_dict=1)
            for a in amt:
                row = [str(b.posting_date)[5:7], "PBVA", a.amount, 0, a.amount, a.amount,
                       0, 0, a.amount, a.tax_amount, 0, b.receipt_number, b.receipt_date]
                data.append(row)
...
def construct_query_bak(filters=None):
    query = """select a.month, a.gross_pay,
	(select b.amount from `tabSalary Detail` b where salary_component = 'Basic Pay' and b.parent = a.name) as basic_pay,
	(select b.amount from `tabSalary Detail` b where salary_component = 'Salary Tax' and b.parent = a.name) as tds ,
	(select b.amount from `tabSalary Detail` b where salary_component = 'PF' and b.parent = a.name) as nppf ,
...
def validate_filters(filters):
    ...
    start, end = frappe.db.get_value("Fiscal Year", filters.fiscal_year, [
