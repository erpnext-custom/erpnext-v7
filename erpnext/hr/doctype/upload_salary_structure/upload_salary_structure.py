...
def add_header(w):
	...
	w.writerow(["Employee", "Employee Name", "Basic", "Corporate",  "Contract", "Communication", "Fuel", "MPI", "Temporary Transfer Allowance", 
"Basic Pay Arrears","Salary Arrears", "PF", "Officiating", "Shift", "PSA", "PDA", "Deputation", "Scarcity", "Difficulty", "High Altitude", "Cash Handling", "Component 1", "Amount 1", "Scheme 1", "Bank 1", "Number 1",  "Component 2", "Amount 2", "Scheme 2", "Bank 2", "Number 2", "Component 3", "Amount 3", "Scheme 3", "Bank 3", "Number 3", "Component 4", "Amount 4", "Scheme 4", "Bank 4", "Number 4", "Component 5", "Amount 5", "Scheme 5", "Bank 5", "Number 5", "Component 6", "Amount 6", "Scheme 6", "Bank 6", "Number 6", "Component 7", "Amount 7", "Scheme 7", "Bank 7", "Number 7", "Component 8", "Amount 8", "Scheme 8", "Bank 8", "Number 8", "Component 9", "Amount 9", "Scheme 9", "Bank 9", "Number 9", "Component 10", "Amount 10", "Scheme 10", "Bank 10", "Number 10"])
...
def upload():
	...
	for i, row in enumerate(rows[3:]):
		...
		try:
			ear = ded = 0
			sws_amount = flt(frappe.db.get_value("Salary Component", "SWSS", "default_amount"))
			...
			if d.employee:
				...
				doc.from_date = '2019-12-01'
				#doc.append("deductions", {"salary_component": "PF", "amount": flt(0.0)})
				...
				'''if d.basic_pay_arrears:
					doc.append("earnings", {"salary_component": "Basic Pay Arrears", "amount": flt(d.basic_pay_arrears)})
					ear += flt(d.basic_pay_arrears)'''
				if d.salary_arrears:
					doc.append("earnings", {"salary_component": "Salary Arrears", "amount": flt(d.salary_arrears)})
					ear += flt(d.salary_arrears)
				if d.pf:
					doc.append("deductions", {"salary_component": "PF", "amount": flt(d.pf)})
					ded += flt(d.pf)


				if d.temporary_transfer_allowance:
                                        doc.eligible_for_temporary_transfer_allowance = 1
					doc.temporary_transfer_allowance_method = 'Lumpsum'
				...
				if d.fuel:
					...
					#ear += amount
				if d.mpi:
                                	doc.eligible_for_mpi = 1
                                        doc.mpi = d.mpi
                                        doc.mpi_method = 'Percent'
                                        #amount = flt(d.fuel)
                                        #doc.append("earnings",{"salary_component": "Fuel Allowance", "amount": amount})
                                        #ear += amount
				...
				doc.append("deductions",{"salary_component": "SWSS", "amount": sws_amount})	
				ded += sws_amount
				...
				if not emp.employee_subgroup:
					frappe.throw("No Grade assigned to " + str(emp.employee_name))
                                ...
                                '''
				gis_amount = flt(frappe.

db.get_value("Employee Grade", emp.employee_subgroup, "gis"))
