...
from erpnext.hr.hr_custom_functions import get_month_details, get_employee_group_settings, get_salary_tax, salary_structure_fieldmap
...
class SalaryStructure(Document):
	...
	def make_earn_ded_table(self):
		...
		for ed in tbl_list:
                        sc_list = frappe.db.sql("select * from `tabSalary Component` where `docstatus` != 2 and `type` = '{0}' and `default` = 1".format(tbl_list[ed]), as_dict=True)
                        for sc in sc_list:
                                child = self.append(ed, {})
                                child.salary_component = sc.name
                                child.amount = flt(sc.default_amount)
                                vars(self)[sc.field_name] = 1
                ...
                for ed in ['earnings','deductions']:
                        ...
                        for ed_item in self.get(ed):
                                ...
                                if ed_item.salary_component not in ed_map:
                                        ...
                                        for m in sst_map[ed]:
                                                if m['salary_component'] == ed_item.salary_component and not self.get(m['field_name']):
                        ...
                        for m in sst_map[ed]:
                                calc_amt = 0
                                if ed == 'earnings':
                                        payment_method = frappe.db.get_value("Salary Component", m['salary_component'], "payment_method")
                                        if self.get(m['field_name']):
                                                calc_amt = round(flt(basic_pay)*flt(self.get(m['value_name']))*0.01 if payment_method == "Percent" else flt(self.get(m['value_name'])))
                                                comm_allowance += round(flt(calc_amt) if m['salary_component'] == 'Communication Allowance' else 0)
                                                total_earning += calc_amt
                                                calc_map.append({'salary_component': m['salary_component'], 'amount': calc_amt})
                                else:
                                        if self.get(m['field_name']) and m['salary_component'] == 'Group Insurance Scheme':
                                                gis_amt  = round(flt(settings.get("gis")))
                                                calc_amt = gis_amt
                                                calc_map.append({'salary_component': m['salary_component'], 'amount': flt(gis_amt)})
                                        elif self.get(m['field_name']) and m['salary_component'] == 'PF':
                                                pf_amt   = round(flt(basic_pay)*flt(settings.get("employee_pf"))*0.01)
                                                calc_amt = pf_amt
                                                calc_map.append({'salary_component': m['salary_component'], 'amount': flt(pf_amt)})
                                        elif self.get(m['field_name']) and m['salary_component'] == 'Health Contribution':
                                                health_cont_amt = round(flt(total_earning)*flt(settings.get("health_contribution"))*0.01)
                                                calc_amt = health_cont_amt
                                                calc_map.append({'salary_component': m['salary_component'], 'amount': flt(health_cont_amt)})
                                        else:
                                                calc_amt = 0
		...
		for d in calc_map['deductions']:
                        if not flt(gross_amt):
                                ...
                                if d['salary_component'] == 'PF':
                                        ...
                                        if not percent:
                                                frappe.throw("Setup PF Percent in Employee Group Settings")
                                ...
                                if d['salary_component'] == 'Health Contribution':
                                        ...
                                        if not percent:
                                                frappe.throw("Setup Health Contribution Percent in Employee Group Settings")
