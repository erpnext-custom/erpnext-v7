...
class Bonus(Document):
	...
	def post_journal_entry(self, cc_amount):
		...
		je.title = "BONUS FOR "+ str(self.fiscal_year) + "(" + str(self.name) + ")"
		...
		query = """
					...
					e.bank_ac_no,
                                        ...
                                        (
                                                ...
                                                and sl.docstatus = 1
                                                and ifnull(ss.eligible_for_annual_bonus,0) = 1
	...
	def on_cancel(self):
		...
		if jv:
			frappe.throw("Can not cancel BONUS without canceling the corresponding journal entry " + str(self.journal_entry))
		else:
