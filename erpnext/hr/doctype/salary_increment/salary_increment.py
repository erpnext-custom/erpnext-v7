...
class SalaryIncrement(Document):
	...
	def validate(self):
		...
		self.check_for_salary_increment() 
	...
	def check_for_salary_increment(self):
		pay_scale = frappe.db.get_value('Employee Grade', self.employee_subgroup, 'maximum')
		if self.new_basic > pay_scale:
			frappe.throw("New basic pay exceeds the pay scale for this grade. Increment not allowed")
