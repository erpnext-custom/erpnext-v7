...
class AssignBranch(Document):
	...
	def assign_branch(self):
		...
		if self.is_volunteer:
			user_perms = frappe.defaults.get_user_permissions(self.user_email)
			for doc, names in user_perms.items():
				if doc == 'Branch':
					for a in names:
						frappe.permissions.remove_user_permission(doc, a, self.user_email)
			frappe.permissions.add_user_permission("Branch", self.volunteer_current_branch, self.user_email)
			for a in self.items:
				frappe.permissions.add_user_permission('Branch', a.branch, self.user_email)
		else:
