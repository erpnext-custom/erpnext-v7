...
class TravelClaim(Document):
    ...
    def validate(self):
        self.get_status()
        hr_role = frappe.db.get_value(
            "UserRole", {"parent": frappe.session.user, "role": "HR Manager"}, "role")
        ...
        if hr_role or (self.workflow_state == "Verified By Supervisor" and frappe.session.user != self.supervisor):  # added hr_role check
            supervisor_d = frappe.get_doc(
                "Employee", {"user_id": self.supervisor})
            frappe.throw("Only <b> {0}. {1} </b>  can verify/edit".format(
                supervisor_d.salutation, supervisor_d.employee_name))
        ...
        if frappe.session.user == self.owner or frappe.session.user == employee:
            ...
            self.sendmail(frappe.db.get_value("Employee", {"user_id": self.supervisor}, "name"), "Travel Claim Submitted", str(
                self.employee_name) + " has requested you to verify and sign a " + str(frappe.get_desk_link("Travel Claim", self.name)))
        elif self.claim_status == "Rejected by Supervisor":
            self.sendmail(self.employee, "Travel Claim Rejected by Supervisor" + str(self.name),
                          "Following remarks has been added by the supervisor: \n" + str(self.reason))
        elif self.claim_status == "Rejected by HR":
            self.sendmail(self.employee, "Travel Claim Rejected by HR" + str(self.name),
                          "Following remarks has been added from HR: \n" + str(self.reason))
    ...
    def on_submit(self):
        ...
        self.sendmail(self.employee, "Travel Claim Approved" + str(self.name), "Your " + str(frappe.get_desk_link(
            "Travel Claim", self.name)) + " has been approved and sent to Accounts Section. Kindly follow up.")

    def before_cancel(self):
        cl_status = frappe.db.get_value(
            "Journal Entry", self.claim_journal, "docstatus")
        if cl_status < 2:
    ...
    def on_cancel(self):
        self.sendmail(self.employee, "Travel Claim Cancelled by HR" + str(self.name),
                      "Your travel claim " + str(self.name) + " has been cancelled by the user")
            ...
            if int(to_monthyear) >= int(from_monthyear):
                for y in range(int(from_year), int(to_year)+1):
                    ...
                    for m in range(int(m_start), int(m_end)+1):
                        ...
                        m_start_date = i.date if str(
                            y)+str(m).rjust(2, str('0')) == str(from_year)+str(from_month) else m_start_date
                        m_end_date = i.till_date if str(
                            y)+str(m).rjust(2, str('0')) == str(to_year)+str(to_month) else get_last_day(m_start_date)
                        if counts.has_key(key):
                            counts[key] += date_diff(m_end_date,
                                                     m_start_date)+1
                        else:
                            counts[key] = date_diff(m_end_date, m_start_date)+1
            else:
                frappe.throw(_("Row#{0} : Till Date cannot be before from date.").format(
                    i.idx), title="Invalid Data")
    ...
    def validate_dsa_ceiling(self):
        ...
        cond3 = ""
        format_string = ""
        lastday_dsa_percent = frappe.db.get_single_value(
            "HR Settings", "return_day_dsa")

        if self.place_type.lower().replace("-", "") == "incountry":
            max_days_per_month = frappe.db.get_single_value(
                "HR Settings", "max_days_incountry")
            if max_days_per_month:
                tt_list = frappe.db.sql_list(
                    "select travel_type from `tabHR Settings Incountry`")
        else:
            max_days_per_month = frappe.db.get_single_value(
                "HR Settings", "max_days_outcountry")
            if max_days_per_month:
                tt_list = frappe.db.sql_list(
                    "select travel_type from `tabHR Settings Outcountry`")

        if tt_list:
            format_string = (
                "'"+"','".join(['%s'] * len(tt_list))+"'") % tuple(tt_list)
            cond1 += "and t1.travel_type in ({0}) ".format(
                format_string, self.travel_type)
        ...
        if max_days_per_month and (not tt_list or self.travel_type in (format_string)):
            ...
            for k in local_count:
                cond2 += " '{0}' between date_format(t2.`date`,'%Y%m') and date_format(ifnull(t2.`till_date`,t2.`date`),'%Y%m') or".format(
                    k)
            ...
            for k, v in local_count.iteritems():
                mapped_count[k] = {'local': v, 'claimed': claimed_count.get(
                    k, 0), 'balance': flt(max_days_per_month)-flt(claimed_count.get(k, 0))}
                ...
                for k, v in record_count.iteritems():
                    ...
                    if mapped_count[k]['balance'] >= v:
                        ...
                        if mapped_count[k]['balance'] < 0:
                            ...
                            i.days_allocated = flt(
                                i.days_allocated) + mapped_count[k]['balance']
                            mapped_count[k]['balance'] = 0

                        if lapsed:
                            counter += 1
                            frappe.msgprint(_("Row#{0}: You have crossed the DSA({4} days) limit by {1} days for the month {2}-{3}").format(
                                i.idx, int(lapsed), months[int(str(k)[4:])-1], str(k)[:4], max_days_per_month))
                            i.remarks = str(i.remarks)+"{3}) {0} Day(s) lapsed for the month {1}-{2}\n".format(
                                int(lapsed), months[int(str(k)[4:])-1], str(k)[:4], counter)
        else:
            for i in self.get("items"):
                i.remarks = ""
    ...
    def update_amounts(self):
        ...
        for i in self.get("items"):
            exchange_rate = 1 if i.currency == company_currency else get_exchange_rate(
                i.currency, company_currency)
            ...
            i.amount = (flt(i.days_allocated)*(flt(i.dsa)*flt(i.dsa_percent)/100)
                        ) + (flt(i.mileage_rate) * flt(i.distance))
        ...
        self.balance_amount = flt(self.total_claim_amount) + \
            flt(self.extra_claim_amount) - flt(self.advance_amount)

        if flt(self.balance_amount) < 0:
            frappe.throw(
                _("Balance Amount cannot be a negative value."), title="Invalid Amount")
    ...
    def check_double_dates(self):
        if self.items:
            ...
            tas = frappe.db.sql("select a.name from `tabTravel Claim` a, `tabTravel Claim Item` b where a.employee = %s and a.docstatus = 1 and a.name = b.parent and (b.date between %s and %s or %s between b.date and b.till_date or %s between b.date and b.till_date) and a.name != %s", (str(
                self.employee), str(start_date), str(end_date), str(start_date), str(end_date), str(self.name)), as_dict=True)
            if tas:
                frappe.throw(
                    "The dates in your current Travel Claim has already been claimed in " + str(tas[0].name))
    ...
    def post_journal_entry(self):
        cost_center = frappe.db.get_value(
            "Employee", self.employee, "cost_center")
        if not cost_center:
            frappe.throw(
                "Setup Cost Center for employee in Employee Information")
        ...
        expense_bank_account = frappe.db.get_value(
            "HR Accounts Settings", None, "employee_payable_account")
        ...
        expense_account = frappe.db.get_single_value(
            "HR Accounts Settings", gl_account)
        if not expense_account:
            frappe.throw(
                "Setup Travel/Training Accounts in HR Accounts Settings")

        je = frappe.new_doc("Journal Entry")
        je.flags.ignore_permissions = 1
        je.title = "Travel Claim (" + self.employee_name + \
            "  " + self.name + ")"
        ...
        if (self.advance_amount) > 0:
            advance_account = frappe.db.get_single_value(
                "HR Accounts Settings", "employee_advance_travel")
            if not advance_account:
                frappe.throw(
                    "Setup Advance to Employee (Travel) in HR Accounts Settings")
            if flt(self.balance_amount) <= 0:
    ...
    def update_travel_authorization(self):
        ...
        if ta.travel_claim and ta.travel_claim <> self.name:
            frappe.throw("A travel claim <b>" + str(ta.travel_claim) +
                         "</b> has already been created for the authorization")
    ...
    def check_status(self):
        if self.supervisor_approval == 1 and self.hr_approval == 1:
            ...
            frappe.throw(
                "Both Supervisor and HR has to approve to submit the travel claim")
    ...
    def validate_dates(self):
        if self.ta_date > self.posting_date:
            frappe.throw(
                "The Travel Claim Date cannot be earlier than Travel Authorization Date")
    ...
    def validate_submitter(self):
        hr_role = frappe.db.get_value(
            "UserRole", {"parent": frappe.session.user, "role": "HR User"}, "role")
        if not hr_role:
    ...
    def sendmail(self, to_email, subject, message):
        ...
        if email:
            try:
                frappe.sendmail(recipients=email, sender=None,
