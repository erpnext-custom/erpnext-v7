...
from frappe.utils import flt, cint, today, nowdate, getdate, get_first_day, add_months, date_diff
...
class SalaryAdvance(Document):
	...
	def validate_amounts(self):
		if flt(self.total_claim) > flt(self.max_advance_limit):
			...
			frappe.throw(_("Advance amount cannot be a negative value"), title="Invalid Data")
		elif flt(self.total_claim) == 0:
			frappe.throw(_("Advance amount should be greater than zero"), title="Invalid Data")
		else:
			pass
	...
	def get_basic_salary(self):
		if self.employee:
			...
			if self.employee_group:
				self.max_months_limit = frappe.db.get_value("Employee Group", self.employee_group, "salary_advance_max_months")
			else:
			...
			self.monthly_deduction = round(flt(self.total_claim) / self.deduction_month)
		else:
			self.basic_pay         = 0
			self.max_months_limit  = 0
	...
	def check_months_served(self):
		x = frappe.db.get_value('Employee', self.employee, 'date_of_joining')
		diff = date_diff(self.application_date, x) + 1 
		# frappe.msgprint(str(diff))
		if diff <= 365:
			frappe.throw("You need to have one year continous services to be eligible for Salary Advance")
