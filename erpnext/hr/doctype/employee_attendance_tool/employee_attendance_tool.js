...
erpnext.EmployeeSelector = Class.extend({
	...
	make: function(frm, employee) {
		...
		var mark_employee_toolbar = $('<div class="col-sm-12 bottom-toolbar">\
			...
			<button class="btn btn-default btn-mark-tour btn-xs"></button>\
		...
		mark_employee_toolbar.find(".btn-mark-tour")
			.html(__('Mark Tour'))
			.on("click", function() {
				var employee_tour = [];
				$(me.wrapper).find('input[type="checkbox"]').each(function(i, check) {
					if($(check).is(":checked")) {
						employee_tour.push(employee[i]);
					}
				});
				frappe.call({
					method: "erpnext.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
						"employee_list":employee_tour,
						"status":"Tour",
						"date":frm.doc.date,
						"company":frm.doc.company
					},
