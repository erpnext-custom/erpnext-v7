...
def get_conditions(filters):
	...
	if filters.get("cost_center"): conditions += " and exists (select 1 from `tabBranch` where branch = mr.branch and cost_center =  '{0}')".format(filters.get("cost_center"))
        if filters.get("warehouse"): conditions += " and exists ( select 1 from `tabMaterial Request` m, `tabMaterial Request Item` mi where  mi.parent = m.name and mi.warehouse = '{0}' and m.name = mr.name)".format(filters.get("warehouse"))
...
def get_data(filters):
    ...
    query = frappe.db.sql("""
			...
			(select mr_item.warehouse from `tabMaterial Request Item` mr_item where mr_item.parent = mr.name limit 1) as warehouse,
