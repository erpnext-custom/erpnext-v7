...
class AssetIssueDetails(Document):
    ...
    def on_submit(self):
        ...
        asset.asset_issue_detail_code = self.name
    ...
    def on_update_after_submit(self):
        asset_code = frappe.db.get_value("Asset", {'asset_issue_detail_code':self.name}, 'name')
        equip_code = frappe.db.get_value("Equipment", {'asset_code':asset_code}, 'name')
        check = frappe.db.sql("select equipment_number, name from `tabEquipment` where equipment_number = '{}'".format(self.reg_number), as_dict=True)
        if check:
            frappe.throw(_("{}" + " already exist for equipment " + "{}").format(check[0].equipment_number, check[0].name))

        doc = frappe.get_doc('Equipment', equip_code)
        doc.equipment_number = self.reg_number
        doc.engine_number = self.engine_no
        doc.chassis_number = self.chesis_no
        doc.save()
