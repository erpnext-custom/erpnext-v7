...
class PurchaseOrder(BuyingController):
	...
	def check_budget_available(self):
		for a in self.items:
			...
			if budget_amount:
				consumed = frappe.db.sql("select SUM(cb.amount) as total from `tabConsumed Budget` cb where cb.cost_center=%s and cb.account=%s and icb.date between %s and %s and exists (SELECT 1 FROM `tabPurchase Order` po WHERE po.name = cb.po_no AND po.docstatus = 1 UNION SELECT 1 FROM `tabJournal Entry` je WHERE je.name = cb.po_no AND je.docstatus = 1)", (a.cost_center, a.budget_account, str(frappe.utils.nowdate())[0:4] + "-01-01", str(frappe.utils.nowdate())), as_dict=True)
				if consumed:
